...
const Call = () => {
  ...
  // 🎵 AUDIO — Synthesized via Web Audio API (avoids CDN 403 errors)
  const audioCtxRef = useRef(null);
  const ringtoneNodeRef = useRef(null);
  const ringbackNodeRef = useRef(null);
  ...
  // Creates a looping beep tone. Returns a stop() function.
  const createTone = (frequency, pattern = [0.4, 0.2]) => {
    ...
    const schedule = () => {
      ...
      gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + pattern[0]);
      osc.connect(gain);
      gain.connect(ctx.destination);
      osc.start(ctx.currentTime);
      osc.stop(ctx.currentTime + pattern[0]);
      osc.onended = () => setTimeout(schedule, pattern[1] * 1000);
    };
    schedule();
  ...
  useEffect(() => {
    return () => {
      ...
      if (ringtoneNodeRef.current) { ringtoneNodeRef.current.stop(); ringtoneNodeRef.current = null; }
      if (ringbackNodeRef.current) { ringbackNodeRef.current.stop(); ringbackNodeRef.current = null; }
  ...
  useEffect(() => {
    // Recipient hears Ringtone (high tone, fast beep)
    if (callStatus === "incoming") {
      if (!ringtoneNodeRef.current) ringtoneNodeRef.current = createTone(880, [0.3, 0.15]);
    } else {
      if (ringtoneNodeRef.current) { ringtoneNodeRef.current.stop(); ringtoneNodeRef.current = null; }
    }

    // Caller hears Ringback (lower tone, slow beep)
    if (callStatus === "calling" || callStatus === "ringing" || callStatus === "connecting") {
      if (!ringbackNodeRef.current) ringbackNodeRef.current = createTone(440, [0.4, 0.6]);
    } else {
      if (ringbackNodeRef.current) { ringbackNodeRef.current.stop(); ringbackNodeRef.current = null; }
    }
  ...
  // Runs ONCE on mount. location.state is stable at mount time.
  // Do NOT add startCall/acceptCall/callStatus to deps — that causes re-runs
  // every time the stream updates (via the cleanup dep chain) which re-triggers
  // startCall after the user ends the call.
  const hasInitialized = useRef(false);
  useEffect(() => {
    if (hasInitialized.current) return;
    const state = location.state;
    if (state?.autoCall && state?.selectedUser) {
      hasInitialized.current = true;
      startCall(state.selectedUser);
    } else if (state?.autoAccept && state?.incomingCallData) {
      hasInitialized.current = true;
      acceptCall(state.incomingCallData);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
